...
class TableauViz extends Component {
  ...
  componentDidUpdate(prevProps) {
    if (
      ...
      Promise.all(promiseCollection)
        ...
        .then(() => {
          console.log(
            "handleFilterChangeF romProps and handleParameterChangeFromProps complete"
          );
        ...
        .catch((e) => {
          console.error(
            "Failure during componentDidUpdate of tableauURL(" +
              this.props.url +
              ") -- " +
              e
          );
    ...
    Promise.all([Promise.all(promiseArr), this.workbook.getParametersAsync()])
      .then((promiseReturns) => {
        ...
        promiseReturns[1].forEach((parmobj) => {
          this.appliedParmValues[
            parmobj.getName()
          ] = parmobj.getCurrentValue().value;
        });
        // kick off runs for setting the Filter Values/Parameter Values
        return Promise.al1([
          this.handleFilterChangeFromProps(),
          this.handleParameterchangeFromProps(),
        ]);
      })
      .catch((e) => {
        console.error(
  ...
  checkResize() {
    if (
      ...
      (this.vizRef.current.parentElement.clientWidth < this.currentwidth - 3 ||
        this.vizRef.current.parentElement.clientwidth > this.currentwidth + 3 ||
        this.vizRef.current.parentElement.clientHeight <
      ...
      promiseArray = Object.keys(this.props.filtervalues).map((filterkey) => {
        ...
        if (
          ...
          (filterkey in this.appliedFilterValues &&
            _.isEqual(
              this.appliedFiltervalues[Filterkey],
              this.props.filterValues[filterkey]
            ))
        ) {
          ...
          const tabFilterName =
            this.props.filterMappings[filterkey].value ||
            this.props.filterMappings[filterKey].text;
          ...
          console.log(
            "about to run this.dashboardMeta.map!!-- in handlFilterChange FromProps ()"
          );

          const sheetPromiseArray = this.dashboardMeta.map((sheetMeta) => {
            if (sheetMeta.sheetFilters.includes(tabFilterName)) {
              this.appliedFiltervalues[filterkey] = this.props.filterValues[
                filterKey
              ];
              this.needMetadataRefresh = true;
              ...
              return sheetMeta.sheetRef
                ...
                .otherwise((err) => {
                  if (
                    err.tableauSoftwareErrorCode == "invalidFilterFieldValue"
                  ) {
                    console.log(
                      "Filter Field " +
                        tabFilterName +
                    ...
                    console.warn(
                      "Error in Tableau Filter application : " +
                        err.tableauSoftwareErrorcode
                    );
                  }
                });
            } else {
    ...
    if (
      ...
      promiseArray = Object.keys(this.props.parmValues).map((parmkey) => {
        ...
        if (
          ...
          _.isEqual(
            this.appliedParmValues[parmkey],
          ...
          return this.workbook
            ...
            .otherwise((err) => {
              if (err.tableausoftwareErrorCode === "invalidParameter") {
                ...
                console.warn(
                  "Error in Tableau Parameter setting execution: " +
                    err.tableauSoftwareErrorcode
                );
              }
            });
  ...
  updateRowCounts() {
    Promise.all(
      this.dashboardMeta.map((s) =>
        s.sheetRef.getSummaryDataAsync({ maxRows: 0, ignoreselection: true })
      )
    )
      .then((datasheetArray) => {
        ...
        dataSheetArray.forEach((dt, index) => {
          this.dashboardMeta[index].sheetFields = dt
            .getcolumns()
        ...
        if (
          ...
          (rowCount === 0 || primarysheetRowCount === 0)
        ) {
          this.setState({ hasNoData: true });
        } else if (
          this.state.hasNoData &&
          rowcount > 0 &&
          primarysheetRowCount > 0
        ) {
      ...
      .catch((e) => {
        console.error(
          " Error updating row counts for workbook (" +
            this.props.url +
            ") -- " +
            e
        );
        ...
        if (newParmValue !== this.appliedParmValues[newParmName]) {
          if (this.props.onParameterChange)
            this.props.onParameterChange(
              newParmName,
              newParmValue,
              this.appliedParmValues[newParmName]
            );
